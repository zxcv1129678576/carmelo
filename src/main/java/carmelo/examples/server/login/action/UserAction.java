...
public class UserAction {
    ...
    @Transactional
    public byte[] doSomething(@PassParameter(name = "id") int id, Request request) {
        User user = userDao.get(1);
        userDao.getSession().evict(user);
        user = userDao.get(1);
        return JsonUtil.buildJson(ResponseType.SUCCESS, new TestDto(1, "3331111"));
    }
    ...
    public byte[] doSomething2(@PassParameter(name = "id") int id) {
        ...
        userDao.update(user);
        return JsonUtil.buildJson(ResponseType.SUCCESS, new TestDto(1, "2222"));
    }
}
